...
5）对目录{data_path_src}下的每张图片文件， 先保持长宽比例不变的情况下压缩到640分辨，调用豆包大模型{doubao-seed-2-0-lite-260428} （采用的API_KEY是{api_key}）,   进行目标检测，检测目标是{req}, 得到目标的yolo风格的bounding-box 的列表。
6）根据该bounding-box, 缩放回在原图中的像素位置，并在原图片中标上加粗的红框，并保存为标上红框的图片，把图片保存到目录'{data_path_target}/labeled/' 中
