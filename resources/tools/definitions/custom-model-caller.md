...
> 校验规则：`api_key`、`provider`、`base_url` 至少提供其一——
> 全空时无法定位 Key 来源，返回 failed 并提示补 provider。
...
- provider / api_key / base_url 全空 → status:failed，提示传 provider 以走 `.env` 密钥
- `.env` 中无该服务商 Key 且未显式传入 → status:failed，message 引导添加 `.env` 条目
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 0.1.1 | 2026-09-02 | api_key 改为可选：未传时自动读 `.env` 的 `<PROVIDER>_API_KEY`（副 key）或主 key；新增 provider/base_url/model 至少其一的校验 |
