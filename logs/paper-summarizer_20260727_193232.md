...
```
stdout:
{"status": "failed", "message": "数据集 'memmem' 的存储路径不存在或无法访问: ", "output_format": "text", "data": {}}
...
def execute(**kwargs) -> dict[str, Any]:
    ...
    try:
        ...
        ds_result = _call_api("api-data-get", name=dataset)  # 根据API规范传入 name 参数
        # 检查返回结果是否包含数据集信息
        if not isinstance(ds_result, dict) or "dataset" not in ds_result:
            return {
                "status": "failed",
        ...
        ds_info = ds_result["dataset"]
        data_path = ds_info.get("path", ds_info.get("storage_path", ds_info.get("directory", "")))
        if not data_path or not os.path.isdir(data_path):
            return {
                "status": "failed",
                "message": f"数据集 '{dataset}' 的存储路径不存在或无法访问: {data_path}",
                "output_format": "text",
                "data": {}
            }
        ...
        model = key_result.get("model", "deepseek-chat")  # 默认为 deepseek-chat
...
=== EXECUTION RESULT ===
stdout: {"status": "failed", "message": "数据集 'memmem' 的存储路径不存在或无法访问: ", "output_format": "text", "data": {}}
stderr: 
=== END RESULT ===
...
def execute(**kwargs) -> dict[str, Any]:
    ...
    try:
        ...
        # 兼容多种 API 返回格式
        ds_info = ds_result.get("data") or ds_result.get("dataset") or ds_result
        if not isinstance(ds_info, dict):
            return {
                "status": "failed",
                "message": f"数据集 '{dataset}' 的接口返回数据格式异常",
                "output_format": "text",
                "data": {}
            }
        ...
        data_path = (
            ds_info.get("path") or
            ds_info.get("storage_path") or
            ds_info.get("directory") or
            ds_info.get("location") or
            ""
        )

        # 仍未获取到路径时使用默认数据目录
        if not data_path:
            default_path = _DATA_DIR / "datasets" / dataset
            if default_path.is_dir():
                data_path = str(default_path)
            else:
                alt_path = _DATA_DIR / dataset
                if alt_path.is_dir():
...
```
stdout:
{"status": "failed", "message": "数据集 'memmem' 的存储路径不存在或无法访问。已尝试默认位置: /Users/leo/Documents/1重要备份/project/2026SOTABand/SOTABand/data/datasets/memmem, /Users/leo/Documents/1重要备份/project/2026SOTABand/SOTABand/data/memmem, /Users/leo/Documents/1重要备份/project/2026SOTABand/SOTABand/data/downloads/memmem 等，均未找到。", "output_format": "text", "data": {}}
...
```
stdout:
{"status": "failed", "message": "数据集 'memmem' 的存储路径不存在或无法访问。已尝试默认位置: /Users/leo/Documents/1重要备份/project/2026SOTABand/SOTABand/data/datasets/memmem, /Users/leo/Documents/1重要备份/project/2026SOTABand/SOTABand/data/memmem, /Users/leo/Documents/1重要备份/project/2026SOTABand/SOTABand/data/downloads/memmem 等，均未找到。API返回信息: {\"id\": \"memmem\", \"name\": \"memmem\", \"version\": \"0.1.0\", \"type\": \"generic\", \"status\": \"active\", \"spec_path\": \"definitions/memmem.md\", \"data_path\": \"/Users/leo/Documents/1重要备份/project/2026SOTABand/SOTABand/data/download/20260726_170036\", \"file_count\": 4, \"total_size\": 4449989, \"formats\": [\"pdf\", \"csv\"]", "output_format": "text", "data": {}}
