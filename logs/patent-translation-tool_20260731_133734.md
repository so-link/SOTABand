...
```
stdout:
{"status": "failed", "message": "专利数量(n)必须为正整数", "output_format": "text", "data": {}}
...
def execute(**kwargs) -> dict[str, Any]:
    ...
    # 尝试从返回数据中提取路径（兼容常见字段名）
    dataset_obj = data_info.get("dataset", {}) if isinstance(data_info, dict) else {}
    data_path = dataset_obj.get("path") or dataset_obj.get("directory") or dataset_obj.get("local_path")
    if not data_path:
        return {"status": "failed", "message": f"数据集 '{dataset}' 未返回有效路径", "output_format": "text", "data": {}}
...
=== EXECUTION RESULT ===
stdout: {"status": "failed", "message": "专利数量(n)必须为正整数", "output_format": "text", "data": {}}
stderr: 
=== END RESULT ===
...
```
stdout:
{"status": "failed", "message": "数据集 '无人机' 未返回有效路径", "output_format": "text", "data": {}}
...
=== EXECUTION RESULT ===
stdout: {"status": "failed", "message": "数据集 '无人机' 未返回有效路径", "output_format": "text", "data": {}}
stderr: 
=== END RESULT ===
...
def execute(**kwargs) -> dict[str, Any]:
    ...
    if isinstance(data_info, dict):
        # 直接查找常见路径键
        for key in ('path', 'directory', 'local_path', 'dir'):
            if key in data_info and isinstance(data_info[key], str) and data_info[key].strip():
                data_path = data_info[key]
                break
        if not data_path:
            # 检查 'dataset' 子对象
            ds = data_info.get('dataset')
            if isinstance(ds, dict):
                for key in ('path', 'directory', 'local_path', 'dir'):
                    if key in ds and isinstance(ds[key], str) and ds[key].strip():
                        data_path = ds[key]
                        break
        if not data_path:
            # 检查 'data' 或 'result' 子对象
            for sub_key in ('data', 'result'):
                sub = data_info.get(sub_key)
                if isinstance(sub, dict):
                    for key in ('path', 'directory', 'local_path', 'dir'):
                        if key in sub and isinstance(sub[key], str) and sub[key].strip():
                            data_path = sub[key]
                            break
                    if data_path:
                        break
...
def execute(**kwargs) -> dict[str, Any]:
    ...
    # 5. 如果上述提取失败，尝试默认下载目录作为备选
    if not data_path:
        default_path = _DOWNLOADS_DIR / dataset
        if default_path.exists() and default_path.is_dir():
            data_path = str(default_path)

    # 6. 最终路径验证
    if not data_path:
        return {"status": "failed", "message": f"数据集 '{dataset}' 未返回有效路径且默认路径不存在", "output_format": "text", "data": {}}
...
```
stdout:
{"status": "failed", "message": "数据集 '无人机' 未返回有效路径且默认路径不存在", "output_format": "text", "data": {}}
...
=== EXECUTION RESULT ===
stdout: {"status": "failed", "message": "数据集 '无人机' 未返回有效路径且默认路径不存在", "output_format": "text", "data": {}}
stderr: 
=== END RESULT ===
...
def execute(**kwargs) -> dict[str, Any]:
    ...
    if isinstance(data_info, str):
        ...
        def find_path(obj, depth=0):
            """递归查找第一个包含路径的字符串值，最大深度5"""
            if depth > 5:
                return None
            if isinstance(obj, dict):
                for key in ('path', 'directory', 'local_path', 'dir', 'location', 'file_path', 'folder'):
                    if key in obj and isinstance(obj[key], str) and obj[key].strip():
                        return obj[key]
                for v in obj.values():
                    res = find_path(v, depth + 1)
                    if res:
                        return res
            elif isinstance(obj, list):
                for item in obj:
                    res = find_path(item, depth + 1)
                    if res:
                        return res
            return None
        data_path = find_path(data_info)
