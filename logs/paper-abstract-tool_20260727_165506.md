...
```
stdout:
{"status": "failed", "message": "参数 'n' 必须为正整数，表示每篇摘要的最大字数"}
...
def execute(**kwargs) -> dict[str, Any]:
    ...
    try:
        ...
        if not ds_response or "dataset" not in ds_response:
            return {"status": "failed", "message": f"数据集 '{dataset}' 不存在或获取信息失败"}
        dataset_info = ds_response["dataset"]
        data_path_str = dataset_info.get("path", "")
        if not data_path_str or not Path(data_path_str).exists():
            return {"status": "failed", "message": f"数据集路径无效或不存在: {data_path_str}"}
        ...
        if not key_response or "api_key" not in key_response:
            return {"status": "failed", "message": "无法获取 DeepSeek API KEY，请检查配置"}
...
=== EXECUTION RESULT ===
stdout: {"status": "failed", "message": "参数 'n' 必须为正整数，表示每篇摘要的最大字数"}
stderr: 
=== END RESULT ===
...
```
stdout:
{"status": "failed", "message": "数据集路径无效或不存在: "}
...
=== EXECUTION RESULT ===
stdout: {"status": "failed", "message": "数据集路径无效或不存在: "}
stderr: 
=== END RESULT ===
...
def execute(**kwargs) -> dict[str, Any]:
    ...
    try:
        ...
        if "dataset" in ds_response and isinstance(ds_response["dataset"], dict):
            data_path_str = ds_response["dataset"].get("path", "")
        elif "data" in ds_response and isinstance(ds_response["data"], dict):
            data_path_str = ds_response["data"].get("path", "")
        # 如果仍然为空，尝试直接从顶层获取
        if not data_path_str:
            data_path_str = ds_response.get("path", "")
        # 再次尝试一些常见别名
        if not data_path_str:
            for key in ("data_path", "file_path", "download_path", "folder"):
                if key in ds_response:
                    ...
                    break
        if not data_path_str and "dataset" in ds_response:
            # 如果 dataset 是一个字符串，可能是名称而不是字典
            if isinstance(ds_response["dataset"], str):
                data_path_str = ds_response.get("path", "")
...
```
stdout:
{"status": "failed", "message": "无法从数据集 'memmem' 的 API 响应中解析出路径。API 响应内容: {\"dataset\": {\"id\": \"memmem\", \"name\": \"memmem\", \"version\": \"0.1.0\", \"type\": \"generic\", \"status\": \"active\", \"spec_path\": \"definitions/memmem.md\", \"data_path\": \"/Users/leo/Documents/1重要备份/project/2026SOTABand/SOTABand/data/download/20260726_170036\", \"file_count\": 4, \"total_size\": 4449989, \"formats\": [\""}
